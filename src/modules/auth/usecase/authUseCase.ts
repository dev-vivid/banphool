...
const { AUDIT_ACTIONS, AUDIT_STATUS } = require('../../../constants');
...
const register = async (body, req) => {
  ...
  await writeAudit({
    action: AUDIT_ACTIONS.REGISTER, resource: 'users', resourceId: user.id,
    req, newValues: { name: user.name, email: user.email, role: user.role },
  });
...
const login = async ({ email, password }, req) => {
  ...
  if (!valid) {
    await writeAudit({
      action: AUDIT_ACTIONS.FAILED_LOGIN, resource: 'users',
      req, status: AUDIT_STATUS.FAILURE, newValues: { email },
    });
    throw { statusCode: 401, message: 'Invalid email or password' };
  }

  if (!user.isActive) throw { statusCode: 401, message: 'Account deactivated' };
...
const refresh = async ({ refreshToken }) => {
  ...
  catch { throw { statusCode: 401, message: 'Invalid or expired refresh token' }; }

  const stored = await tokenService.findValidRefreshToken(decoded.id);
  if (!stored) throw { statusCode: 401, message: 'Refresh token revoked or expired' };

  return { accessToken: tokenService.signAccess({ id: decoded.id, email: decoded.email, role: decoded.role }) };
};
